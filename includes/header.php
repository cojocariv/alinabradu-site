...
<head>
  ...
  <meta property="og:url" content="<?= e((isset($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost') . currentUrlPath()) ?>">
  ...
  <header class="bg-white/90 backdrop-blur border-b border-gold/20 sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-6 text-sm uppercase tracking-wide">
        ...
        <a href="/magazin" class="hover:text-gold">Magazin</a>
        <a href="/despre-noi" class="hover:text-gold">Despre noi</a>
        <a href="/contact" class="hover:text-gold">Contact</a>
        <a href="/cos" class="hover:text-gold">Coș (<?= cartItemsCount() ?>)</a>
      </nav>
    </div>
  </header>
