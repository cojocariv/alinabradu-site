...
$orderMessage = productOrderMessage(
    (string) $product['name'],
    (string) $product['slug'],
    (string) ($productImages[0] ?? $product['image'])
);
...
<section class="max-w-7xl mx-auto px-4 py-10 grid md:grid-cols-2 gap-10">
  <div>
    <div class="space-y-3">
      <div class="rounded-lg bg-[#fffaf2] border border-[#eadfc9] p-3">
      ...
      <?php if (count($productImages) > 1): ?>
        <div class="flex flex-wrap gap-2 justify-center">
          <?php foreach ($productImages as $i => $imgUrl): ?>
            <button type="button" class="product-thumb border-2 rounded overflow-hidden w-20 h-20 bg-[#fff6ea] shrink-0 <?= $i === 0 ? 'border-gold' : 'border-transparent opacity-80 hover:opacity-100' ?>" data-src="<?= e($imgUrl) ?>" aria-label="Imagine <?= $i + 1 ?>">
              <img src="<?= e($imgUrl) ?>" alt="" class="w-full h-full object-contain p-1 bg-[#fff6ea]">
            </button>
          <?php endforeach; ?>
        </div>
      <?php endif; ?>
  ...
  <div>
    ...
    <form method="post" class="space-y-4">
      ...
      <div class="product-page-actions">
        ...
        <?php require __DIR__ . '/../includes/product_order_chat_buttons.php'; ?>
      </div>
    </form>
    <?php else: ?>
    <form action="<?= e(url('/contact')) ?>" method="get" class="space-y-4">
      ...
      <div class="product-page-actions">
        <button type="submit" class="bg-zinc-900 text-white px-6 py-3 rounded hover:bg-zinc-700">Adaugă în coș</button>
        <?php
        $whatsAppLabel = 'Solicitare la comandă';
        $viberLabel = 'Solicitare pe Viber';
        require __DIR__ . '/../includes/product_order_chat_buttons.php';
        ?>
      </div>
    </form>
    <?php endif; ?>
